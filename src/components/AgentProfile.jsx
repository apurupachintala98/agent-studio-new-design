...
//         // Extract unique providers for LangGraph
//         if (isLangGraph && data.length > 0) {
//           const uniqueProviders = [...new Set(data.map((m) => m.provider || m.model_provider).filter(Boolean))];
//           setLlmProviders(uniqueProviders);
//           if (uniqueProviders.length > 0 && !llmServiceProvider) {
//             setLlmServiceProvider(uniqueProviders[0]);
//           }
//         }
//       } catch (error) {
//         console.error("Failed to load LLM models:", error);
...
//         {/* Response Instruction - Cortex only */}
//         {agentType === "Cortex" && (
//           <div className="mt-6 flex items-start">
//             <label className="text-sm flex-shrink-0 pr-3 pt-3" style={{ color: "#37474F", fontWeight: 500 }}>Response Instruction</label>
//             <textarea
...
//   const [responseInstruction, setResponseInstruction] = useState("");
//   const [toolChoice, setToolChoice] = useState("auto");
//   const [llmServiceProvider, setLlmServiceProvider] = useState("");
