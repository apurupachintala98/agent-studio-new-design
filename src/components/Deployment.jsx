...
function DeploymentStepper({ onClear }) {
  ...
  const initDeployment = async () => {
    ...
    interval = setInterval(async () => {
      try {
        const data = await callPipelineAPI();
        if (data.status.toLowerCase() === "completed") {
          setMessageState({ message: "Deployment Completed", percentage: 100 });
          clearInterval(interval);
          ...
          setMessageState((s) => ({ message: "Deploying to EKS", percentage: Math.min(s.percentage + 1, 99) }));
        }
      } catch (err) {
    ...
    >
      <div className="flex items-start">
        ...
        <div className="flex flex-col items-center" style={{ width: 260, flexShrink: 0 }}>
          ...
          <div className="w-full mt-2 rounded-full overflow-hidden" style={{ height: 8, backgroundColor: "#E0E0E0", maxWidth: 220 }}>
            <div className="h-full rounded-full" style={{ width: `${messageState.percentage}%`, backgroundColor: messageState.message.includes("Failed") ? "#D32F2F" : "#0072C6" }} />
          </div>
          <span
            className="mt-2"
            style={{
              fontSize: messageState.message.includes("Failed") ? 14 : 12,
              fontWeight: messageState.message.includes("Failed") ? 700 : 400,
              color: messageState.message.includes("Failed") ? "#D32F2F" : "#90A4AE",
            }}
          >
  ...
  return (
    ...
    >
      ...
      >
        <div
          className="flex items-center justify-center rounded-2xl"
          style={{ width: 80, height: 80, backgroundColor: "#0D1B2A" }}
        >
          <svg width="40" height="40" viewBox="0 0 40 40" fill="none">
            <path d="M6 12C6 10.9 6.9 10 8 10H16L19 13H32C33.1 13 34 13.9 34 15V30C34 31.1 33.1 32 32 32H8C6.9 32 6 31.1 6 30V12Z" fill="#29B6F6" />
            <rect x="16" y="18" width="3" height="10" rx="1" fill="#0D1B2A" opacity="0.5" />
        ...
        <span className="font-semibold mt-3" style={{ fontSize: 15, color: "#1A1A1A" }}>Source Artifacts</span>
        <span className="text-sm mt-1" style={{ color: "#78909C" }}>
          {isDownloading ? "Downloading..." : `agent_${isLangGraph ? "langgraph" : "cortex"}_v1.zip`}
        </span>
        <span className="text-xs mt-0.5" style={{ color: "#90A4AE" }}>
...
export default function Deployment({ onFinish, agentDetails }) {
  ...
  const callLogAPI = async (d1, d2, d3) => {
    return fetch(`https://aedl-devops.edl.dev.awsdns.internal.das/tekton/s3-logs/${d1}/${d2}/${d3}`)
      .then(res => res.json());
  };

  // Called by DeploymentStepper when deployment completes or fails
  const onDeploymentClear = async (data) => {
    try {
      const logData = await callLogAPI("EDA-EKS-NP1-Cluster", "bmbpoc-srv-devops", data.name);
      setLogs([logData.log_content]);
    } catch (err) {
  ...
  const handleChatWithAgent = () => {
    if (!agentUuid) return;
    localStorage.setItem("agentName", agentDetails?.agnt_nm || "");
    localStorage.setItem("agentId", agentUuid);
    localStorage.getItem("session_id");
    localStorage.getItem("user_id");
    localStorage.getItem("aplctn_cd");
    navigate("/chat");
  ...
  // Finish Deployment — no API call, just marks as finished and enables Chat
  const handleFinishDeployment = async () => {
    setIsFinishing(true);
    try {
      setIsFinished(true);
  ...
  return (
    <>
      <div className="mt-6">
        <SectionHeader>Deployment Progress</SectionHeader>
        <DeploymentStepper onClear={onDeploymentClear} />
      </div>
