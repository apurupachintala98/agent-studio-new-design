...
export default function MemoryConfig({
  ...
  // Memory selection
  const [shortTermSelected, setShortTermSelected] = useState(
    memDefaults?.short_term_memory_needed ?? true
  );
  const [longTermSelected, setLongTermSelected] = useState(
    memDefaults?.long_term_memory_needed ?? false
  );

  // Long-term sub-types (multi-select)
  const [semanticMemory, setSemanticMemory] = useState(
    memDefaults?.long_term_memory_config?.semantic_user_profile ?? false
  );
  const [episodicMemory, setEpisodicMemory] = useState(
    memDefaults?.long_term_memory_config?.episodic_user_experience ?? false
  );
  const [proceduralMemory, setProceduralMemory] = useState(
    memDefaults?.long_term_memory_config?.procedural_user_instructions ?? false
  );
