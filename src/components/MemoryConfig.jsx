...
export default function MemoryConfig({
  ...
  // Restore from savedData if user already configured this step
  const savedMem = savedData?.memoryConfig;

  // Memory selection — restore from savedData first, then defaults
  const [shortTermSelected, setShortTermSelected] = useState(savedMem?.short_term_memory_needed ?? true);
  const [longTermSelected, setLongTermSelected] = useState(savedMem?.long_term_memory_needed ?? false);

  // Long-term sub-types (multi-select)
  const [semanticMemory, setSemanticMemory] = useState(savedMem?.long_term_memory_config?.semantic_user_profile ?? false);
  const [episodicMemory, setEpisodicMemory] = useState(savedMem?.long_term_memory_config?.episodic_user_experience ?? false);
  const [proceduralMemory, setProceduralMemory] = useState(savedMem?.long_term_memory_config?.procedural_user_instructions ?? false);
