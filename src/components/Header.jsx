...
export default function Header() {
  ...
  // Determine the subtitle based on route
  const getSubtitle = () => {
    if (location.pathname.startsWith('/cortex-agent')) return 'AGENT STUDIO - CORTEX';
    if (location.pathname.startsWith('/langraph-agent')) return 'AGENT STUDIO - LANGGRAPH';
    return 'AGENT STUDIO';
  };

  const handleLogout = () => {
    localStorage.removeItem('user_id');
    localStorage.removeItem('aplctn_cd');
    navigate('/');
  };
  ...
  return (
    <header style={{ background: '#FFF', width: '100%', height: '59px', display: 'flex', alignItems: 'center', borderBottom: '1px solid #E5E7EB' }}>
      ...
      >
        ...
        {/* Agent Studio Text - Dynamic based on route */}
        <div
          style={{
            color: '#1A3673',
