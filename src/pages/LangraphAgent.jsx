...
export default function LangGraphAgent() {
  ...
  useEffect(() => {
    ...
    const loadAgentDetails = async () => {
      try {
        const response = await langgraphApi.getAgentDetails(agentId);
        const record = response?.data?.record;
        setAgentDetails(record);
                localStorage.setItem("agentType", record?.agnt_type || "LangGraph");


        const config = {
          profile_config: record?.profile_config || null,
          memory_config: record?.memory_config || null,
          tool_config: record?.tool_config || null,
          backend_config: record?.backend_config || null,
          frontend_config: record?.frontend_config || null,
        };
  ...
  // Helper to build common identifiers
  const getCommonIds = () => {
    ...
    try {
      ...
      const profilePayload = {
        ...
        llm_config: {
          ...
          provider_name: data.llmProviderName || data.llmServiceProvider || "",
          llm_auth: {
